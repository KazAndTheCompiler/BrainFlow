...
class KnowledgeStore:
    ...
    def _auto_extract_tags(self, text: str) -> list:
        """Auto-extract topic tags from text using heuristics."""
        text_lower = text.lower()
        tags = []

        code_indicators = {"def ", "function", "class ", "import ", "const ", "let ",
                          "var ", "return ", "if (", "for (", "while (", "async ",
                          "await ", "=>", "->", "//", "/*", "===", "!==", "=="}
        ui_indicators = {"button", "click", "submit", "login", "search", "cancel",
                        "delete", "save", "close", "ok", "apply", "menu", "nav",
                        "settings", "profile", "home", "window", "panel"}
        data_indicators = {"number", "count", "total", "sum", "average", "result",
                           "output", "value", "data", "record", "file", "error"}

        if any(ind in text_lower for ind in code_indicators):
            tags.append("code")
        if any(ind in text_lower for ind in ui_indicators):
            tags.append("ui")
        if any(ind in text_lower for ind in data_indicators):
            tags.append("data")
