...
class ScreenObserver:
    ...
    def __init__(self, feature_dim=256, capture_interval=2.0,
        ...
        # Screen UI pipeline for semantic analysis
        self.screen_ui_engine = None
        self._last_ui_summary = ""
        self._last_ui_layout = None  # type: Optional[object]

    def attach_brain(self, brain, text_encoder, knowledge_store, screen_ui_engine=None):
        """Wire up the brain so OCR text flows into neural input + memory."""
        self.brain = brain
        self.text_encoder = text_encoder
        self.knowledge_store = knowledge_store
        self.screen_ui_engine = screen_ui_engine
    ...
    def _capture_and_process(self):
        """Capture screen, run UI analysis, inject into brain + knowledge store.

        Uses ScreenUIEngine when available for semantic UI understanding.
        Falls back to simple motion + OCR when not.
        """
        ...
        if self.screen_ui_engine is not None:
            try:
                ...
                layout = self.screen_ui_engine.analyze(
                    screenshot,
                ...
                self._last_ui_summary = encoded["summary"]

                if self.brain and encoded["text_features"].size > 0:
                    self.brain.inject_sensory_input("text", encoded["text_features"])
                if self.brain and encoded["vision_features"].size > 0:
                    self.brain.inject_sensory_input("vision", encoded["vision_features"])
                if self.brain and "ui_features" in encoded and encoded["ui_features"].size > 0:
                    self.brain.inject_sensory_input("ui", encoded["ui_features"])
                if self.brain:
            ...
            if self.brain and self.text_encoder and ocr_text.strip():
                try:
                    if len(ocr_text.strip()) >= 10:
                        text_features = self.text_encoder.encode(ocr_text[:500])
    ...
    def get_state(self):
        ...
        state = {
            ...
            "ui_pipeline_active": self.screen_ui_engine is not None,
        }
        if self._last_ui_layout is not None:
            state["ui_summary"] = self._last_ui_summary
            state["ui_regions"] = len(self._last_ui_layout.regions)
            state["ui_dominant_type"] = self._last_ui_layout.dominant_type.value
        return state
