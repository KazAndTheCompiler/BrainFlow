...
class ClaudeBridge:
    ...
    def send_observation(self, observation: dict):
        ...
        if obs_type == "text" and isinstance(content, str):
            if self._text_encoder is None:
                ...
                self._text_encoder = TextEncoder(feature_dim=256)
            features = self._text_encoder.encode(content)
            ...
            if isinstance(content, str):
                if self._text_encoder is None:
                    from sensory.text import TextEncoder
                    self._text_encoder = TextEncoder(feature_dim=256)
                features = self._text_encoder.encode(f"ACTION: {content}")
                self.brain.inject_sensory_input("text", features)
    ...
    def poll_brain_events(self) -> list[dict]:
        """
        Called periodically from the simulation loop to detect notable brain events.
        Returns a list of event dicts that the server can forward to Claude.
        This enables Claude to respond to brain events, not just serve requests.
        """
        events = []
        ...
        try:
            ...
            if nm.get("dopamine", 0) > 0.85:
                events.append({
                    "type": "high_dopamine",
                    "message": "Brain dopamine high — prediction error / novelty detected",
                    "step": brain_state.get("step", 0),
                })

            if nm.get("acetylcholine", 0) > 0.85:
                events.append({
                    "type": "high_attention",
                    "message": "Brain acetylcholine high — strong attention/learning",
                    "step": brain_state.get("step", 0),
                })

            pred_region = regions.get("predictive", {})
            if pred_region and pred_region.get("surprise", 0) > 0.5:
                events.append({
                    "type": "high_surprise",
                    "message": f"Predictive layer surprise: {pred_region['surprise']:.2f} — brain detecting novelty",
                    "step": brain_state.get("step", 0),
                })

            hipp = regions.get("hippocampus", {})
            if hipp and hipp.get("replay_mode", False):
                events.append({
                    "type": "memory_replay",
                    "message": f"Hippocampus in replay mode — {hipp.get('memories_stored', 0)} memories stored",
                    "step": brain_state.get("step", 0),
                })

            reflex = regions.get("reflex_arc", {})
            if reflex and reflex.get("reflex_active", False):
                events.append({
                    "type": "reflex_active",
                    "message": "Reflex arc active — fast stimulus-response triggered",
                    "step": brain_state.get("step", 0),
                })

            stage = brain_state.get("development_stage", "")
            if stage and stage != getattr(self, "_last_reported_stage", None):
                events.append({
                    "type": "stage_change",
                    "message": f"Brain development stage: {stage}",
                    "step": brain_state.get("step", 0),
                })
