...
class SensoryCortex(BrainRegion):
    """Processes raw sensory input (vision, audio, touch, UI region types).
    ...
    def __init__(self, n_neurons: int, dt: float = 1.0):
        ...
        quarter = n_neurons // 4
        self.vision_range = (0, quarter)
        self.audio_range = (quarter, 2 * quarter)
        self.touch_range = (2 * quarter, 3 * quarter)
        self.ui_range = (3 * quarter, n_neurons)
    ...
    def encode_ui(self, ui_features: np.ndarray) -> np.ndarray:
        """Encode UI region type vector into spike currents.
        ui_features: [button, input, text_block, code, nav, icon, image, table, unknown]
        """
        current = np.zeros(self.n_neurons)
        u_start, u_end = self.ui_range
        n_ui = u_end - u_start
        if len(ui_features) > 0:
            mapped = np.interp(
                np.linspace(0, 1, n_ui),
                np.linspace(0, 1, len(ui_features)),
                ui_features
            )
            current[u_start:u_end] = mapped * 12.0
        return current
